...
# Can be used both as a sourced library or as a standalone CLI tool.
#
# Compatible with bash 3.2+ and zsh
#
# Usage:
#   ./packaging-format.sh detect --cpack-generator <GENERATOR>
#   ./packaging-format.sh detect --os-name <OS_NAME>
#
# Examples:
#   ./packaging-format.sh detect --cpack-generator NSIS
#   ./packaging-format.sh detect --os-name windows-latest
#   ./packaging-format.sh detect --os-name Linux    # from ${{ runner.os }}
#   source ./packaging-format.sh && daq_packaging_detect_from_os "ubuntu-latest"
...
# Detect if script is being sourced or executed (compatible with bash and zsh)
if [[ -n "${BASH_VERSION:-}" ]]; then
    # bash: compare BASH_SOURCE[0] with $0
    ...
    # Fallback: try to return, if it fails we're not sourced
